...
namespace NSPM_Yacht
{
    public class Yacht : Script
    {
        ...
        public Ped NPCSpawn(string sPed, Vector3 vLocal, float fAce, int iTask)
        {
            ...
            Ped BuildPed = ObjectBuild.NPCSpawn(sPed, vLocal, fAce, false, 140, 0, 0, null, 0, false, 0, 0, "");
        ...
        private void YachtStuff_StartOnYacht()
        {
            ...
            int iStart = RandomNum.FindRandom(209, 1, 5);
            if (iStart == 1)
            {
        ...
        private void YachtStuff_TheBlip(bool bOn)
        {
            ...
            if (bOn)
            {
                if (YachtBlip == null)
                {
                    YachtBlip = ObjectBuild.AddBlips(vYachtBlip);
                            ...
                            else
                            {
                                if (!Function.Call<bool>(Hash.GET_IS_TASK_ACTIVE, Peddy, 134))
                                {
                                    if (Peddy.Gender == Gender.Female)
                                    {
                                        if (!bBacktoBase)
                                        {
                                            int iDell = RandomNum.RandInt(0, 4);
                                            if (iDell == 0)
                        ...
                        for (int i = 0; i < vYachtDoorList.Count; i++)
                        {
                            ...
                            if (PlayPos.DistanceTo(ThisDoor) < 1.50f)
                            {
                                ...
                                if (ReturnStuff.WhileButtonDown(47))
                                {
                                    ...
                                    iDoors = iDoors + 1;
                                    if (iDoors > RandomNum.RandInt(25, 50))
                                    {
                                        TheMissions.SlowFastTravel(vRandomDestList[RandomNum.RandInt(0, vRandomDestList.Count - 1)], 0.00f);
                                        iDoors = 0;
                                    }
                                    else
                                    {
                                        int iDoor = RandomNum.RandInt(0, vYachtDoorList.Count - 1);
                                        while (iDoor == i)
                                        {
                                            iDoor = RandomNum.RandInt(0, vYachtDoorList.Count - 1);
                                            Script.Wait(100);
                                        }
                                        Vector3 MyDoor = new Vector3(vYachtDoorList[iDoor].X, vYachtDoorList[iDoor].Y, vYachtDoorList[iDoor].Z);
                                        TheMissions.SlowFastTravel(MyDoor, vYachtDoorList[iDoor].R);
                                    }
                                }
                            }
                        }
        ...
        private void YachtStuff_YachtDrink()
        {
            ...
            else
            {
                ...
                if (RandomNum.FindRandom(206, 1, 4) < 2)
                {
                    ...
                    int iPosy = RandomNum.FindRandom(207, 0, VectorList_01.Count - 1);
                ...
                else
                {
                    ...
                    int iPosy = RandomNum.FindRandom(208, 0, VectorList_01.Count - 1);
                    Vector3 Vspot = VectorList_01[iPosy];
                    Vector3 VRota = new Vector3(0.00f, 0.00f, fList_01[iPosy]);
                    Game.Player.Character.Position = Vspot;
                    Function.Call(Hash.POPULATE_NOW);
        ...
        private void YachtStuff_YachtParty()
        {
                ...
                if (ThisProp.Exists())
                {
                    if (ThisProp.Model == Function.Call<int>(Hash.GET_HASH_KEY, "hei_prop_yah_seat_02") || ThisProp.Model == Function.Call<int>(Hash.GET_HASH_KEY, "hei_prop_yah_seat_03"))
                    {
                        if (RandomNum.RandInt(0, 20) < 10)
                        {
                            ...
                            ObjectBuild.PedSitHere(Psit, ThisProp, 1);
                        }
                    }
                    else if (ThisProp.Model == Function.Call<int>(Hash.GET_HASH_KEY, "hei_prop_yah_lounger"))
                    {
                        ...
                        {
                            Ped Psit = NPCSpawn(ReturnStuff.RandNPC(12), ThisProp.Position, ThisProp.Heading - 180f, 0);
                            ObjectBuild.PedSitHere(Psit, ThisProp, 2);
                        }
                    }
                    else if (ThisProp.Model == Function.Call<int>(Hash.GET_HASH_KEY, "hei_prop_yah_seat_01"))
                    {
                        ...
                        {
                            Ped Psit = NPCSpawn(ReturnStuff.RandNPC(12), ThisProp.Position, ThisProp.Heading - 180f, 0);
                            ObjectBuild.PedSitHere(Psit, ThisProp, 8);
                        }
                    }
                }
            }
            ...
            for (int i = 1; i < Pos_01.Count; i++)
            {
                if (i < 6)
                {
                    int iRanPeds = RandomNum.RandInt(2, 3);
                    for (int ii = 0; ii < iRanPeds; ii++)
                    {
                        Vector3 VPedPos = Pos_01[i].Around(2.50f);
                        VPedPos.Z = Pos_01[i].Z;
                        Ped Psit = NPCSpawn(ReturnStuff.RandNPC(12), VPedPos, RandomNum.RandInt(0, 360), 3);
                ...
                else
                {
                    if (i == Pos_01.Count - 1)
                    {
                        for (int ii = 0; ii < 9; ii++)
                        {
                            ...
                            Ped DancinF = NPCSpawn(ReturnStuff.RandNPC(12), VPedPos, RandomNum.RandInt(0, 360), 0);
                    ...
                    {
                        int iRanPeds = RandomNum.RandInt(4, 9);
                        for (int ii = 0; ii < iRanPeds; ii++)
                        {
        ...
        private void YachtStuff_DancePedDance()
        {
            if (iKeepDance < Game.GameTime)
            {
                ...
                {
                    bool bMale = false;
                    int iRan = RandomNum.RandInt(1, 3);
        ...
        private void PlayerDance()
        {
            ...
            if (ReturnStuff.WhileButtonDown(47))
            {
                if (iYachtDrink > 1)
                {
                    ...
                    DancinFool.Add(DrString[RandomNum.FindRandom(210, 0, DrString.Count - 1)]);
